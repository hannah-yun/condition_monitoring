...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Script for Case I and II (Number of iterations are the same) for AKSVD1
...
vibdata = ndata(:,2:end); % exclude frequency column
...
for j=1:length(facn)
    [vEBIC(:,j),vdist(:,j),vbests(:,j),vt(:,j),bestD] = DL_AKSVD1_simulation(vibdata,nu,N,facn(j),vs);
end
toc;
