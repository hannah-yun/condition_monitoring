    ...
    [~, f] = pwelch(x_input(:,1), 2048,1024,2048, Fs); % length 2048
    itr = size(x_input, 2);
    ...
    for i = 1:itr
        ...
        [Pxx, ~] = pwelch(signal, 2048,1024,2048,Fs);
    ...
    % Slice data: keep only frequencies ≤ 250 Hz (adjust this value if needed) 
    inds = psd_welch(:,1) <= 250;

    % Keep only the rows corresponding to selected frequency range
    slice_data = psd_welch(inds,:);
    
    % Save sliced data to .mat file
    save("case2_sliced_welch.mat", 'slice_data');
