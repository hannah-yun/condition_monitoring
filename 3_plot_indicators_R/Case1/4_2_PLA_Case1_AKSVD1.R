...
load("_results/_NREL_NEW_CODE/AKSVD1/_NEW_NREL_AKSVD1_1000_2500_3500.RData")

# Define a function to apply piecewise linear approximation using the cpop package
# dataset: a list of data frames containing AKSVD1 results
# seg_num: minimum segment length (used by cpop)
cpop_nrel_AKSVD1 <- function(dataset,seg_num){
  master_list <- list()
  
  for(i in 1:9){
    
    slice_df <- dataset[[i]]$AKSVD1_df  #Extract the ith dataset
...
AKSVD1_seed_nrel <- cpop_nrel_AKSVD1(AKSVD1_nrel, 0)
...
for (i in seq_along(AKSVD1_seed_nrel)) {
  AKSVD1_seed_nrel[[i]][[2]]$bearings <- bearings[i]        # Add bearing label
  AKSVD1_seed_nrel[[i]][[2]]$cutoff_freq <- cutoff[i]       # Add cutoff frequency
}
...
list_fitted_AKSVD1 <- list()
list_total_AKSVD1 <- list()

# Extract fitted and original data frames from results
for(tb in 1:length(AKSVD1_seed_nrel)){
  list_fitted_AKSVD1[[tb]] <- as.data.frame(AKSVD1_seed_nrel[[tb]][2])
  list_total_AKSVD1[[tb]] <- as.data.frame(AKSVD1_seed_nrel[[tb]][1])
}



################ Piecewise Linear Approximation with AKSVD1 ################ 

# Create a new list with only the fitted data frames for AKSVD1 algorithm
AKSVD1_fitted <- lapply(seq_along(list_fitted_AKSVD1), function(i) {
  fit_df <- list_fitted_AKSVD1[[i]]
  return(fit_df)
})

# Combine all fitted data frames into a single data frame
fitted_df_AKSVD1 <- do.call(rbind, AKSVD1_fitted)

# Ensure 'index' is treated as a factor for plotting/grouping
fitted_df_AKSVD1$index <- factor(fitted_df_AKSVD1$index)

# Create custom legend labels based on 'index' factor levels
fitted_tb_label <- paste("AN", levels(fitted_df_AKSVD1$index))
